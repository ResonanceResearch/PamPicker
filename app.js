...
function renderLegend() {
  el.legend.innerHTML = `
    ...
    <span class="legend-item"><span class="legend-swatch" style="background:${cssVar('--gold')}"></span>Forward gene</span>
    <span class="legend-item"><span class="legend-swatch" style="background:${cssVar('--danger')}"></span>Reverse gene</span>
  `;
}
...
function renderGenome() {
  ...
  const visibleSites = state.filteredSites;

  const baseWidth = Math.max(1200, Math.ceil(record.length * Math.max(0.18, state.zoom * 0.18)));
  const width = Math.min(Math.max(baseWidth, el.viewerContainer.clientWidth - 4), 60000);
  const height = 390;
  const leftPad = 70;
  const rightPad = 40;
  const drawable = width - leftPad - rightPad;
  const scale = drawable / record.length;
  const geneTrackY = 120;
  const axisY = 82;
  const pamTopY = 210;
  const pamBottomY = 320;
  const geneLanes = allocateFeatureLanes(features);

  el.viewerInner.style.width = `${width}px`;
  ...
  const axis = `
    ...
    ${buildTicks(record.length, leftPad, axisY, drawable)}
  `;

  const featureSvg = features.map((feature, idx) => {
    const lane = geneLanes[idx];
    return drawFeature(feature, lane, leftPad, geneTrackY, scale);
  }).join('');

  const pamSvg = visibleSites.map(site => drawPamMarker(site, leftPad, pamTopY, pamBottomY, scale, state.selectedIds.has(site.id), el.showNumbers.checked)).join('');
...
function buildTicks(length, leftPad, axisY, drawable) {
  const roughStep = length / 8;
  const step = niceTick(roughStep);
  const ticks = [];
  for (let pos = 1; pos <= length; pos += step) {
    const x = leftPad + ((pos - 1) / length) * drawable;
    ticks.push(`
...
function drawFeature(feature, lane, leftPad, geneTrackY, scale) {
  const y = geneTrackY + lane * 34;
  const x1 = leftPad + (feature.start - 1) * scale;
  const x2 = leftPad + feature.end * scale;
  const width = Math.max(8, x2 - x1);
  const color = feature.strand >= 0 ? cssVar('--gold') : cssVar('--danger');
  const arrowSize = Math.min(18, width * 0.4);
  ...
  return `
    <g class="feature-box">
      ...
      <text x="${Math.max(x1 + 3, Math.min((x1 + x2) / 2, x2 - 3))}" y="${y - 4}" text-anchor="middle" class="feature-label">${label}</text>
    </g>
  `;
}
...
function drawPamMarker(site, leftPad, topY, bottomY, scale, selected, showNumbers) {
  const x = leftPad + ((site.pamStart + site.pamEnd) / 2) * scale;
